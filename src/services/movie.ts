...
import { FilmInfo, getWatchReturnedType, Item, Reviews, Video } from "../shared/types";
    ...
    const movieInfo = response.reduce((final, current, index) => {
        switch (index) {
            ...
            case 2:
                ...
                break;
            case 3:
                final.reviews = current.data.results.map((item: Item) => ({
                    ...item,
                    media_type: "movie",
                }));
                break;
...
export const getWatchMovie = async (id: number): Promise<getWatchReturnedType> => {
    const res = await Promise.all([axios.get(`/movie/${id}`), axios.get(`/movie/${id}/recommendations`)]);
