...
import { DetailSeason, FilmInfo, getWatchReturnedType, Item, Reviews, Video } from "../shared/types";

export const getTVFullDetail = async (id: number): Promise<FilmInfo> => {
    const response = await Promise.all([axios.get(`/tv/${id}`), axios.get(`/tv/${id}/credits`), axios.get(`/tv/${id}/reviews`), axios.get(`/tv/${id}/similar`), axios.get(`/tv/${id}/videos`)]);
...
export const getWatchTV = async (id: number): Promise<getWatchReturnedType> => {
    const res = await Promise.all([axios.get(`/tv/${id}`), axios.get(`/tv/${id}/recommendations`)]);
    ...
    const detailSeasons = (await Promise.all(data.detail.seasons.map((season: DetailSeason) => axios.get(`/tv/${id}/season/${season.season_number}`)))).map((res) => res.data);
