            ...
            {SortReview(reviews, type).map((review) => (
                <li key={review.id} className="flex gap-7">
                    <div className="shrink-0 max-w-[60px] w-full h-[60px]">
                        <LazyLoadImage src="/me.jpg" alt="reviewer" effect="opacity" className="w-[60px] h-[60px] rounded-full object-cover" />
                    </div>
    ...
    return (
        <>
            <div className="flex gap-4 justify-end -mt-5 mb-10">
                <p>Sort Rating: </p>
                <select className="outline-none bg-inherit" value={reviewSortType} onChange={(e) => setReviewSortType(e.target.value)}>
