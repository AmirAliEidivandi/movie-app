    ...
    return (
        <>
            <div className="flex gap-10 text-gray-400 text-lg justify-center">
                <button
                    className={`hover:text-white transition duration-300 pb-1  ${currentTab === "overall" && "font-medium -translate-y-2 border-b-2 border-primary text-white"}`}
                    onClick={() => setCurrentTab("overall")}
                >
                    Overall
                </button>
                <button
                    className={`hover:text-white transition duration-300 pb-1 ${currentTab === "cast" && "font-medium -translate-y-2 border-b-2 border-primary text-white"}`}
                    onClick={() => setCurrentTab("cast")}
                >
                    Cast
                </button>
                <button
                    className={`hover:text-white transition duration-300 pb-1 ${currentTab === "reviews" && "font-medium -translate-y-2 border-b-2 border-primary text-white"}`}
                ...
                {detail && detail.media_type === "tv" && (
                    <button
                        className={`hover:text-white transition duration-300 pb-1 ${currentTab === "seasons" && "font-medium -translate-y-2 border-b-2 border-primary text-white"}`}
                    ...
                    <Skeleton className="w-[40%] h-4 mb-1" />
                        ...
                        {detail && (
                            <>
                                ...
                                {detail.media_type === "movie" && <p>Release date: {(detail as DetailMovie).release_date}</p>}
                                {detail.media_type === "tv" && <p>Last air date: {(detail as DetailTV).last_air_date}</p>}
                                <p>
                                    Spoken language:
                            ...
                            credits.map((cast) => (
                                <li key={cast.id} className="flex gap-3 items-center">
                                    <div className="shrink-0 max-w-[65px] w-full h-[65px]">
                                        <LazyLoadImage src={resizeImage(cast.profile_path, "w185")} alt="" effect="opacity" className="object-cover rounded-full h-[65px] w-[65px]" />
                                    </div>
                                ...
                                <li key={season.id}>
                                    <div className="flex gap-3 items-center">
                                        <div className="shrink-0 max-w-[120px] w-full">
                                            <LazyLoadImage src={resizeImage(season.poster_path, "w92")} alt="" effect="opacity" className="object-cover w-[120px] h-full rounded-md" />
                                        </div>
                                        <div className="flex-grow">
