...
const SignUp: FunctionComponent<SignUpProps> = ({ setIsSignIn, isSignIn }) => {
    ...
    return (
        <>
            ...
            {error && <ModalNotification type="error" message={error} setError={setError} />}
            <div className="px-4 py-2 rounded-xl max-w-xl w-full min-h-[500px] text-white/70 absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2">
                <div className="flex flex-col items-center mb-5">
                    ...
                    <div className="flex gap-4 mb-8">
                        ...
                        <button
                            onClick={() => SignInWithProvider(new FacebookAuthProvider(), "facebook")}
                            className="h-12 w-12 rounded-full bg-white tw-flex-center hover:brightness-75 transition duration-300"
                        >
                    ...
                    <Form>
                        <div className="flex md:justify-between gap-8 md:gap-0 mb-6">
                            <div className="relative">
                                <Field name="firstName" type="text" placeholder="First name" className="w-full bg-dark-lighten px-5 py-4 pr-12 rounded-xl outline-none peer text-white" />
                                <label
                                    htmlFor="firstName"
                ...
                `}
                            ...
                            <div className="relative">
                                <Field name="lastName" type="text" placeholder="Last name" className="w-full bg-dark-lighten px-5 py-4 pr-12 rounded-xl outline-none peer text-white" />
                                <label
                                    htmlFor="lastName"
                                    className={`absolute left-5 text-gray-400 transition duration-500 pointer-events-none 
                translate-y-[-50%] visible peer-placeholder-shown:opacity-0 peer-placeholder-shown:invisible peer-placeholder-shown:translate-y-[-10%] ease-in-out
                `}
                        ...
                        <div className="relative mb-6">
                            <Field name="email" type="email" placeholder="Email" className="w-full bg-dark-lighten px-5 py-4 pr-12 rounded-xl outline-none peer text-white" />
                            <label
                                htmlFor="email"
                                className={`absolute left-5 text-gray-400 transition duration-500 pointer-events-none 
                translate-y-[-50%] visible peer-placeholder-shown:opacity-0 peer-placeholder-shown:invisible peer-placeholder-shown:translate-y-[-10%] ease-in-out
                ...
                `}
                        ...
                        <button type="submit" className="px-12 py-3 bg-primary rounded-full text-lg text-white uppercase absolute left-1/2 -translate-x-1/2 hover:bg-[#4161cc] transition duration-300">
                            Register
                        </button>
                    </Form>
                </Formik>
                <p className="text-xl flex gap-2 mt-32 justify-center">
                    ...
                    <button type="submit" onClick={() => setIsSignIn(!isSignIn)} className="text-primary/90 underline">
                        Sign In
                    </button>
                </p>
            </div>
