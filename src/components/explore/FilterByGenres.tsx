...
const FilterByGenres: FunctionComponent<FilterByGenresProps> = ({ currentTab }) => {
    const [genres] = useAutoAnimate();
    const { isLoading, data, isError, error } = useQuery<getRecommendGenres2Type, Error>(["genres"], getRecommendGenres2);
    const [searchParam, setSearchParam] = useSearchParams();
    const [currentSearchParams] = useCurrentParams();

    if (isError) return <div>ERROR: {error.message}</div>;
    if (isLoading) return <div className="mt-20 mb-20 mx-auto h-10 w-10 rounded-full border-[5px] border-dark-darken border-t-transparent animate-spin"></div>;

    const chooseGenre = (genreId: string) => {
        const existingGenres = searchParam.getAll("genre");

        if (existingGenres.includes(genreId)) {
            const newGenres = existingGenres.filter((genre: string) => genre !== genreId);
            setSearchParam({
                ...currentSearchParams,
                genre: newGenres,
            });
