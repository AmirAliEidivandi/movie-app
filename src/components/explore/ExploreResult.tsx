...
const ExploreTVResult: FunctionComponent<ExploreTVResultProps> = ({ pages }) => {
    return (
...
const ExploreResult: FunctionComponent<ExploreResultProps> = ({ currentTab, config }) => {
    ...
    } = useInfiniteQuery<ItemsPage, Error>(["explore-result-movie", config], ({ pageParam = 1 }) => getExploreMovie(pageParam, config), {
    ...
    } = useInfiniteQuery<ItemsPage, Error>(["explore-result-tv", config], ({ pageParam = 1, queryKey }) => getExploreTV(pageParam, queryKey[1] as { [key: string]: string }), {
        getNextPageParam: (lastPage) => (lastPage.page + 1 <= lastPage.total_pages ? lastPage.page + 1 : undefined),
    });
    ...
    return (
        <>
            {currentTab === "movie" && (
                <>
                    {movies?.pages.reduce((acc, current) => [...acc, ...current.results], [] as any).length === 0 ? (
                        ...
                        <InfiniteScroll dataLength={movies?.pages.length || 0} next={() => fetchNextPageMovie()} hasMore={Boolean(hasNextPageMovie)} loader={<div>Loading...</div>} endMessage={<></>}>
            ...
            {currentTab === "tv" && (
                <>
                    {tvs?.pages.reduce((acc, current) => [...acc, ...current.results], [] as any).length === 0 ? (
                        <div className="flex flex-col items-center mb-12">
                            <LazyLoadImage src="/error.png" alt="" effect="opacity" className="w-[600px]" />
                            <p className="text-white text-3xl mt-5">There is no such films</p>
                        </div>
                    ) : (
                        <InfiniteScroll dataLength={tvs?.pages.length || 0} next={() => fetchNextPageTv()} hasMore={Boolean(hasNextPageTv)} loader={<div>Loading...</div>} endMessage={<></>}>
